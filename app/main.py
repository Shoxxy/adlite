...
# Import-Weiche (Lokal vs Server)
try:
    from logic import generate_adjust_payload, send_request_auto_detect, get_skadn_value_for_app, ua_manager, extract_id_and_platform_from_link
except ImportError:
    from app.logic import generate_adjust_payload, send_request_auto_detect, get_skadn_value_for_app, ua_manager, extract_id_and_platform_from_link
...
async def startup_event():
    if DISCORD_WEBHOOK_URL:
        try:
            # FIX: Content hinzufügen, sonst Error 400
            DiscordWebhook(url=DISCORD_WEBHOOK_URL, content="🚀 Zone C Engine gestartet & bereit.").execute()
        except: 
            pass
...
def send_detailed_log(status, ctx, resp, ip, proxy):
    ...
    color = "39ff14" if "success" in status else "ff3333"
    try:
        webhook = DiscordWebhook(url=DISCORD_WEBHOOK_URL)
        # Fallback Content für Webhook, falls Embeds failen
        webhook.content = f"Log: {ctx['app_name']} - {status}"
        
        embed = DiscordEmbed(title=f"SDK INJECTION {status.upper()}", color=color)
        embed.set_author(name=f"USER: {ctx.get('username', 'System')}")
        
        embed.add_embed_field(name="Target", value=f"{ctx['app_name']} / {ctx['event_name']}", inline=True)
        embed.add_embed_field(name="Device ID", value=ctx['device_id'], inline=True)
        
        stealth = f"Proxy: {proxy}\n"
        if ip: stealth += f"IP Spoof: {ip}"
        
        embed.add_embed_field(name="Stealth", value=stealth, inline=False)
        
        clean_resp = str(resp)[:800]
        embed.add_embed_field(name="Response", value=f"```\n{clean_resp}\n```", inline=False)
...
async def internal_execute(
    ...
    try:
        ...
        # Wir ignorieren use_get_request aus der Config, da wir fix14.py Logik (POST) erzwingen
        skadn_val = get_skadn_value_for_app(app_name) if platform == "ios" else None

        # 2. Payload generieren (SDK Style aus fix14.py)
        ...
        # 3. Senden (SDK Header Simulation)
