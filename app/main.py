...
def send_detailed_log(status_type, context, response_text, user_agent, is_fixed_ua):
    ...
    # Header Status
    state_suffix = "(LOCKED)" if is_fixed_ua else "(UPDATED)"
    embed = DiscordEmbed(title=f"INJECTION {status_type.upper()} {state_suffix}", color=color)
    ...
    # Korrekte Bezeichnung im Log
    if is_fixed_ua:
        ua_status = "🔒 LOADED FROM DB (Immutable)"
    else:
        ua_status = "♻️ OVERWRITTEN / NEW CREATED"

    embed.add_embed_field(name="UA Status", value=ua_status, inline=True)
...
async def internal_execute(
    ...
    force_ua_update: bool = Form(False), # FastAPI konvertiert "true" String automatisch in True
    x_api_key: str = Header(None)
):
    if x_api_key != INTERNAL_API_KEY: raise HTTPException(status_code=403)

    # 1. UA Holen (is_cached = True, wenn er nicht geändert wurde)
